...
export const confirmAPI = (response) => {
  const { data, err } = response.response.body;
...
export const checkRoadBead = (lotteryId) => {
  return [10, 110, 116, 118].includes(lotteryId);
}

export const checkDoubleLong = (lotteryId) => {
  return [116, 118].includes(lotteryId);
}
