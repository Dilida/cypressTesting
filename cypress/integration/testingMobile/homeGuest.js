...
import apiRoute from '../../support/mobileApiRoute.js'
...
describe('Mobile Home Page', () => {
  ...
  it('首页 header', () => {
    ...
    cy.get(':nth-child(1) > .q-list > :nth-child(1)')
    .should('contain', '充值').click();
    dialogClose();
    cy.get(':nth-child(1) > .q-list > :nth-child(2)').should('contain', '提款').click();
    dialogClose();
  ...
  it('优惠活动', () => {
    cy.wait(['@activity']).then((response) => {
      ...
      .should('be.visible').and('contain', '更多 >')
  ...
  it('帮助中心', () => {
    cy.get('.cooper')
      .should('be.visible').and('contain', '合作加盟')
      .find('.q-icon')
